			...
			try {
				...
				if (config.EnableQQHttp) {
					...
					if (string.IsNullOrEmpty(config.QQHttpToken))
						_logger.LogWarning("No QQ Http token provided, make sure to set it right if token is enabled in your server settings.");
				}
				...
				if (config.EnableQQWebSocket) {
					...
					if (string.IsNullOrEmpty(config.ToQQID))
						throw new Exception(message: "No QQ ID provided!");
					if (string.IsNullOrEmpty(config.QQWebSocketToken))
						_logger.LogWarning("No QQ WebSocket token provided, make sure to set it right if token is enabled in your server settings.");
				}
